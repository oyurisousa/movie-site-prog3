  ...
  return (
    <main className="main">
      {isLoading ? (
        ...
        <>
          <h2>All Films</h2>
          <select title="genres" name="genres" id="genres" onChange={handleGenreChange}>
...
// const getMoviesByGenres = async (page:number | string = 1 ,genres: string[]): Promise<GetMoviesResponse | null>=> {
//   try {
//     const response = await api.get(`/discover/movie?page=${page}&primary_release_year=2024&sort_by=popularity.desc&with_genres=${genres.join('')}
// `);
//     return {
...
//           <h2>All Films</h2>
//           {/* <ByGenres/> */}
//           <select title="genres" name="genres" id="genres">
//             {genres.map((genre)=>{
//               return <option key={`genre-${genre.id}`} value={genre.name}>{genre.name}</option>
//             })}
//           </select>
//           <div>
//             {movies.map((movie, index) => (
